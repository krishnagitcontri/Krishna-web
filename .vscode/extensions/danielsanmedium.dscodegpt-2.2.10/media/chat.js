...
(function () {
  ...
  const newFileWithCodeSvg = '<svg width="1.2rem" height="1.2rem" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="#808080"><path fill-rule="evenodd" clip-rule="evenodd" d="M9.5 1.1l3.4 3.5.1.4v2h-1V6H8V2H3v11h4v1H2.5l-.5-.5v-12l.5-.5h6.7l.3.1zM9 2v3h2.9L9 2zm4 14h-1v-3H9v-1h3V9h1v3h3v1h-3v3z"/></svg>'
  ...
  const syncGithubButtonSignUp = document.querySelector('#btn-sync-github-signup')
  ...
  const getRawMessages = () => {
    const messages = []
    const chats = document.querySelectorAll('div#chat_container.hljs > div > div > div:not(.profile)')
  ...
  function chatStripe (isAi, value, uniqueId, aiName) {
    aiName ??= document.getElementById('currentProvider').value
    return (
      `
            <div class="wrapper ${isAi && 'ai'}">
                <div class="chat">
                    <div class="profile chat_header">
                        <div class="avatar">${isAi ? botSvg : (userPic.value !== 'undefined' && userPic.value) ? `<img src="${userPic.value}" alt="user profile picture">` : userSvg}</div>
  ...
  syncGithubButtonSignUp?.addEventListener('click', syncGithub)
  ...
  function heightChangeHandler (event) {
    ...
    chatContainer.style.marginBottom = '2.5rem' // `min(calc(${target.scrollHeight}px + 3.5rem), 19.5rem)`
  ...
  window.addEventListener('message', event => {
    ...
    switch (message.type) {
      ...
      default: {
        const typePrompt = {
          explainCodeGPT: 'Code Explanation:',
          refactorCodeGPT: 'Code Refactoring:',
          documentCodeGPT: 'Code Documentation:',
          findProblemsCodeGPT: 'Bug Detection:',
          unitTestCodeGPT: 'Unit Testing:',
